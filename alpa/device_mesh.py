...
from alpa.util import (benchmark_func, list_gpu_info, OrderedSet, synchronize_one_event,
...
class MeshHostWorker:
    ...
    def _get_buffers_with_local_ids(self, uuid: int, device_ids: Sequence[int]):
        ...
        if device_ids is None:
            # host_wait_for_events(done_events)
            return bufs
        elif not isinstance(device_ids, Iterable):
            # host_wait_for_events([done_events[device_ids]])
            return bufs[device_ids]
        # host_wait_for_events([done_events[device_id] for device_id in device_ids])
